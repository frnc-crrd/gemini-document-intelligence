...
class DocumentIngestionProcessor:
    ...
    def _should_ignore(self, zip_member_path: str) -> bool:
        """Determina si un archivo debe ser ignorado auditando su jerarquía de rutas."""
        path_parts = Path(zip_member_path).parts
        
        for part in path_parts:
            if any(part.startswith(prefix) for prefix in AppConfig.IGNORED_PREFIXES):
                return True
                
        if Path(zip_member_path).suffix.lower() not in AppConfig.ALLOWED_EXTENSIONS:
            return True
