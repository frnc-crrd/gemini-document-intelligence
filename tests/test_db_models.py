...
def test_registro_artefacto_schema_definition() -> None:
    ...
    # Validación de versionamiento (Nueva regla de negocio)
    ...
    target_constraint = next(
        (c for c in constraints if isinstance(c, UniqueConstraint) and c.name == 'uix_folio_version_origen'), 
        None
    )
    
    assert target_constraint is not None, "La restricción 'uix_folio_version_origen' no fue definida."
